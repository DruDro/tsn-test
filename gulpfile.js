...
var path = {
  ...
  src: {
    ...
    sass: [
      'src/static/styles/main.scss', 'src/static/styles/other/*.scss'
    ],
    img: [
      'src/static/assets/img/**/*.*', '!src/static/assets/img/sprite/**/*.*', '!src/static/assets/img/svg/**/*.*', '!src/static/assets/img/**/*.html'
    ],
    pic: [
      'src/static/assets/pic/**/*.jpg', 'src/static/assets/pic/**/*.png'
    ],
  ...
  watch: {
    html: [
      'src/pages/**/*.html', 'src/modules/**/*.html'
    ],
    svgHtml: 'src/static/assets/img/svg.html',
    sass: 'src/static/styles/**/*.scss',
    fonts: 'src/static/assets/fonts/**/*.*',
    js: [
      'src/static/scripts/**/*.js', 'src/modules/**/*.js'
    ],
    pic: 'src/static/assets/pic/**/*.*',
    img: [
      'src/static/assets/img/**/*.*', '!src/static/assets/img/sprite/**/*.*', '!src/static/assets/img/svg/**/*.*'
    ],
    spr: {
      svg: 'src/static/assets/img/sprite/svg/**/*.svg',
      retina2x: 'src/static/assets/img/sprite/retina/2x/*.png',
...
var webserver = {
  dev: {
    ...
    //tunnel: true, host: 'localhost', port: 9001, logPrefix: 'app_dev'
  },
  prod: {
    server: {
      baseDir: './build'
    },
    //tunnel: true, host: 'localhost', port: 9002, logPrefix: 'app_prod'
  }
};
...
gulp.task('sprite:default', function () {
  var spriteData = gulp
    .src(path.src.spr.default, {read: false})
    .pipe(plumber(plumberCfg))
  ...
  spriteData
    .css
    .pipe(gulp.dest(path.src.styles)); // путь, куда сохраняем стили
  return spriteData;
});

gulp.task('clean:1x', function (cb) {
  return gulp
    .src(path.src.spr.retina1x, {read: false})
    .pipe(clean());
});

gulp.task('retina:resize', ['clean:1x'], function (cb) {
  var ret = gulp
    .src(path.src.spr.retina2x)
    .pipe(plumber(plumberCfg))
...
gulp.task('img:pic', function () {
  ...
  // false}],     use: [pngquant()],     interlaced: true }))
    .pipe(gulp.dest(path.build.pic));
});
//Чистим img
gulp.task('clean:img', function (cb) {
  return gulp
    .src(path.build.img, {read: false})
    .pipe(clean());
});
var imgTimer;
...
gulp.task('clean:svgHtml', function (cb) {
  return gulp
    .src(path.build.svgHtml, {read: false})
    .pipe(clean());
});
...
gulp.task('svgHtml', function () {
  ...
  gulp
    .src(path.src.svgHtml)
    .pipe(plumber(plumberCfg))
    .pipe(fileinclude(fileIncludeCfg))
    .pipe(gulp.dest(path.build.svgHtml))
});

gulp.task('svgSpriteCompl', gulpsync.sync(['sprite:svg', 'svgHtml']));

// ---------------------------------------// Сборка html
// ---------------------------------------// Чистим html
gulp.task('clean:html', function (cb) {
  return gulp
    .src(path.build.html, {read: false})
    .pipe(clean());
});
...
gulp.task('watch', function () {
  ...
  watch([
    path.watch.sass, path.src.modules
  ], function (event, cb) {
